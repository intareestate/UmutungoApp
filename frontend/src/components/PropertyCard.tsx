...
export type PropertyPlaceholder = { id: string; title: string; type: string; location: string; price: string; priceNote: string; bedrooms: number; bathrooms: number; area: number; accent: string; image: string; listed: string };
  ...
  return (
    <article className="property-card">
      <div className="property-image">
        <Image src={property.image} alt={`${t(language, property.title)} ${t(language, 'property image')}`} fill sizes="(max-width: 760px) 100vw, 33vw" className="property-image-art" />
      ...
      <div className="property-card-body">
        ...
        <div className="property-card-footer"><div><strong>{property.price} <small>{t(language, property.priceNote)}</small></strong><small className="property-listed">{t(language, property.listed)}</small></div><button className="card-link" type="button" onClick={onView}>{t(language, 'View property')} <Icon name="arrow" size={14} /></button></div>
      </div>
    </article>
  );
}
